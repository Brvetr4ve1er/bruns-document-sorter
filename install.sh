...
# ── helper: download a tessdata language pack if missing ────────────────────
ensure_traineddata() {
    local lang="$1"
...
if curl -fsS --max-time 3 http://localhost:11434/api/tags >/dev/null 2>&1; then
    echo "      ${GREEN}[OK]${RESET} Ollama is running on http://localhost:11434"
    if curl -fsS --max-time 3 http://localhost:11434/api/tags | grep -q "llama3"; then
        echo "      ${GREEN}[OK]${RESET} llama3 model is available."
    else
        echo "      ${YELLOW}[!]${RESET} llama3 model not found."
        read -r -p "          Pull now? (downloads ~4.7 GB) [y/N]: " yn
        if [[ "${yn:-N}" =~ ^[Yy]$ ]]; then
            ollama pull llama3
        fi
    fi
    ...
    echo
    read -r -p "          Press Enter to continue (Ollama can be added later)..."
fi
echo
